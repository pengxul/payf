...
如果您感兴趣，您可以参考 `\Pengxul\Payf\get_wechat_public_certs` 方法了解详细细节。
...
\Pengxul\Payf\Pay::config($config);

$params = [
    '_config' => 'default' // 多租户配置时使用
];

\Pengxul\Payf\get_wechat_public_certs($params, storage_path('pay/wechat_default'));
```
...
\Pengxul\Payf\get_wechat_public_certs($params);
```
